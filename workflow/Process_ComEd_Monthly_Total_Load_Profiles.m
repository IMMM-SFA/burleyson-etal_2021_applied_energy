...
% Loop over the filelist and aggregate the data by month, time-of-day, and customer class:
counter = 0; i = 0;
for year = 2018:2020
    Progress(1,1) = year;
    for month = 1:12
...
for year = 2018:2020
    for month = 1:12
        ...
        if isempty(Subset_Time) == 0
           ...
           Mean_Load_Time(counter,3) =  max(Weekday_Mean);
           Mean_Load_Time(counter,4) = Time_Vector(1,find(Weekday_Mean == max(Weekday_Mean)));
           Mean_Load_Time(counter,5) =  max(Weekend_Mean);
           Mean_Load_Time(counter,6) = Time_Vector(1,find(Weekend_Mean == max(Weekend_Mean)));
...
% Save the output:
save([data_directory,'/output_data/ComEd_Monthly_Total_Load_Profiles_Test.mat'],'Mean_Load_Time','Mean_Weekday_Load','Mean_Weekend_Load');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
